...
const groups = [
    {
        divider: {key: '1', content: <p>Группа №1</p>},
        options: [
            { key: '1', content: 'Neptunium' },
            { key: '2', content: 'Plutonium' },
        ],
    ...
    {
        divider: {key: '2', content: <p>Группа №2</p>},
        options: [
...
render(<Select options={groups} label='Группированный селект' multiple={true} allowUnselect={true} />);
```
...
const groups = new Array(100).fill().reduce((acc, _, groupIndex) => {
    return acc.concat({
        divider: {key: `${groupIndex + 1}`, content: <p>{`Группа №${groupIndex + 1}`}</p>},
